...
export class CreattorShopComponent implements OnInit {
  ...
  // ✅ Added properties to avoid TS2339 errors
  product_grid: string | null = null;  // For dynamic section title
  product: any = null;                 // For single product view or detail
  products: any[] = [];                // For product grid list

  constructor(
    private shopItems: NftDataService,
    private collectionItems: CollectionDataService
  ) {
  ...
  ngOnInit(): void {
    // Populate products array from shop data (if needed)
    this.products = this.shopItems.shopData();
  }
